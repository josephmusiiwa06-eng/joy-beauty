...
export function CinematicScene({ className = '' }: CinematicSceneProps) {
  ...
  useEffect(() => {
    ...
    const setupScene = () => {
      ...
      // ── Particle System — Floating Petals/Dust ───────────────
      ...
      // ── Floating Geometric Orbs (African-inspired shapes) ─────
      const orbGroup = new THREE.Group();
      scene.add(orbGroup);

      const orbData: { mesh: THREE.Mesh; speed: number; radius: number; angle: number }[] = [];

      const orbConfigs = [
        { pos: [2.5, 1.5, -2], scale: 0.4, color: 0xff1b8d, geo: 'sphere' },
        { pos: [-2.8, -1.0, -3], scale: 0.3, color: 0xd4af37, geo: 'torus' },
        { pos: [0, 2.8, -4], scale: 0.25, color: 0xe8b4cb, geo: 'sphere' },
        { pos: [-1.5, -2.5, -2], scale: 0.35, color: 0xff6bb5, geo: 'torus' },
        { pos: [3.5, -0.5, -5], scale: 0.5, color: 0xc4006a, geo: 'sphere' },
        { pos: [-3.5, 2.0, -6], scale: 0.4, color: 0xf0d060, geo: 'torus' },
      ];
      ...
      orbConfigs.forEach((cfg, i) => {
        ...
        orbData.push({
          mesh,
          speed: 0.2 + i * 0.07,
          radius: Math.sqrt(cfg.pos[0] ** 2 + cfg.pos[1] ** 2),
          angle: Math.atan2(cfg.pos[1], cfg.pos[0]),
        });
      });

      // ── Central Ring — Crown Motif ────────────────────────────
      const ringGroup = new THREE.Group();
      scene.add(ringGroup);

      const outerRingGeo = new THREE.TorusGeometry(1.8, 0.015, 16, 200);
      ...
      const innerRingGeo = new THREE.TorusGeometry(1.3, 0.008, 16, 200);
      ...
      // ── Scroll-based Camera + Scene Animation ─────────────────
      ...
      scrollTimeline
        // Phase 1: Hero — zoom in slowly, rings spin up
        ...
        // Phase 2: Services — camera drifts left, orbs scatter
        ...
        // Phase 3: Gallery — swoop up, particles bloom
